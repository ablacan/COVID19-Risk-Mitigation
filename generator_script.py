...
class Generator():
    ...
    def __logistic_step(self, t, params):
        """
        ...
        """
        C, L, a, b = params
        return C + self.__logistic(t, L, a, b)
    ...
    @staticmethod
    def __generate_sigmoid(NB_SIMUL):
        """
    ...
    @staticmethod
    def __generate_ascent_descent(NB_SIMUL, sigmoid_params, MAX_DAYS, IDX_LAG=3, i=1):
        """
    ...
    def __generate_full_sigmoid(self, NB_SIMUL):
        """
        ...
        sig_params = self.__generate_sigmoid(NB_SIMUL)
        ...
        for i, MAX_DAYS, IDX_LAG in zip(range(1, 4), [random_MAX_DAYS1, random_MAX_DAYS2, random_MAX_DAYS3], [3, 7, 11]):
            sig_params = self.__generate_ascent_descent(NB_SIMUL, sig_params, MAX_DAYS, IDX_LAG, i)

        return sig_params



    def generate(self, NB_SIMUL, add_noise=False):
        ...
        sigmoids_params = self.__generate_full_sigmoid(NB_SIMUL) #Generate parameters for 4 sigmoid steps per simulation

        #Loop over number of simulations to compute stringency data
        for i in range(len(sigmoids_params)):
            log1 = self.__logistic_step(np.arange(1,sigmoids_params[i,3],1), sigmoids_params[i,:4])
            log2 = self.__logistic_step(np.arange(sigmoids_params[i,3],sigmoids_params[i,7],1), sigmoids_params[i,4:8])
            log3 = self.__logistic_step(np.arange(sigmoids_params[i,7], sigmoids_params[i,11],1), sigmoids_params[i,8:12])
            log4 = self.__logistic_step(np.arange(sigmoids_params[i,11],360+1,1), sigmoids_params[i,12:16])
            ...
            #Normalize data
            simulated_stringency = self.__normalize_data(simulated_stringency)
